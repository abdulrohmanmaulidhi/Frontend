...
const normalizePost = (raw: any): CommunityPost => ({
  ...
  author: raw?.author ?? raw?.created_by ?? raw?.nama ?? "Anonim",
...
export async function createCommunityPost(payload: { judul: string; komentar: string; rating: number; tanggal?: string }): Promise<CommunityPost | null> {
  try {
