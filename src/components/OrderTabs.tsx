...
interface OrderTabsProps {
  ...
  onTabChange: (tab: "orders" | "history") => void;
}
...
function Frame({ isActive, onClick }: { isActive: boolean; onClick: () => void }) {
  return (
    <div 
      onClick={onClick}
      className={`${isActive ? "bg-[#b49de4]" : ""} content-stretch flex h-[74px] items-end justify-center px-[10px] py-[23px] relative rounded-[20px] shrink-0 w-[490px] max-md:w-full cursor-pointer transition-all hover:opacity-95`}
    >
      <p className={`basis-0 font-['Inter:Medium',sans-serif] font-medium grow h-full leading-[normal] min-h-px min-w-px not-italic relative shrink-0 text-[24px] max-md:text-[18px] text-center ${isActive ? "text-white" : "text-black"}`}>
        Pesanan Saya
      </p>
    </div>
  );
